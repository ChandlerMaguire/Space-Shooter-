...
class Player(Ship):
    def __init__(self, x, y, health=100):
        ...
        self.mask = pygame.mask.from_surface(self.ship_img) # defines pixel perfect hit box 
...
def main():
    ...
    player_velocity = 5
    laser_velocity = 5
    main_font = pygame.font.SysFont("8-Bit-Madness", 50)
    lost_font = pygame.font.SysFont("8-Bit-Madness", 60)
    ...
    # every iteration of the loop, redraw everything onto screen
    def redraw_window():
        # blit draws image to location specified in parameters
    ...
    while run:
        # clock ticks based on FPS to keep running consistent 
        clock.tick(FPS)
        redraw_window()
        ...
        # checks every tick for if an event has occurred (mouse click, etc,.)
